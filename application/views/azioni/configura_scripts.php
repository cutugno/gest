...
<script type="text/javascript">
	...
	$("#aggiungi_campo").click(function() {
		...
		var nuovo_campo="<tr><td><input type='text' name='campo_descrizione[]' value='' class='form-control'  /></td><td><input type='checkbox' name='a_active' value='1' class='a_active'  /></td><td>{select}</td><td class='text-center'><button type='button' class='btn btn-success aggiorna_campo' data-idcampo='{id_campo}' style='margin: 0 4px 0 -1px;' ><i class='fa fa-pencil-square-o' aria-hidden='true'></i> SALVA CAMPO</button><button type='button' class='btn btn-danger elimina_campo' data-idcampo='{id_campo}' ><i class='fa fa-trash-o' aria-hidden='true'></i> ELIMINA CAMPO</button></td></tr></form>";		
		var select="<select name='a_avvocato' class='a_avvocato form-control' data-idavv='' style='width:350px; display:none'>{options}</select>";
